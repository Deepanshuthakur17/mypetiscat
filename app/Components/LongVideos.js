...
import { 
    X, Home, ThumbsUp, Share2, ChevronLeft, ChevronRight, 
    Grid, List, Sparkles, Heart, Laugh, Crown, Stethoscope, PawPrint, Play, ChevronDown 
} from 'lucide-react';
...
function VideoModal({ video, onClose }) {
    ...
    return (
        ...
        >
            <button onClick={onClose} className="absolute top-6 left-6 z-110 text-white p-3 rounded-full bg-gray-900/80 hover:bg-[#E5C082] hover:text-black transition-all">
                <X size={32} />
            </button>

            <div className="w-full max-w-5xl aspect-video relative bg-black rounded-3xl overflow-hidden shadow-2xl border border-white/10">
                {isLoading && <PawLoader />}
                <iframe 
            ...
            <div className="hidden lg:flex flex-col gap-6 ml-8">
                <button onClick={() => { new Audio('/cat-meow-For-Like-button.mp3').play().catch(()=>{}); setShowLikeAlert(true); }} className="flex flex-col items-center text-[#E5C082] hover:text-white transition-colors">
                    <ThumbsUp size={32} />
                    <span className="text-xs font-bold mt-2 uppercase tracking-widest">Like</span>
                </button>
                <button onClick={() => { navigator.clipboard.writeText(videoUrl); alert("Link Copied!"); }} className="flex flex-col items-center text-[#E5C082] hover:text-white transition-colors">
                    <Share2 size={32} />
                    <span className="text-xs font-bold mt-2 uppercase tracking-widest">Share</span>
                </button>
            </div>

            <AnimatePresence>
                {showLikeAlert && (
                    <motion.div className="absolute z-120 p-8 bg-black border-2 border-[#E5C082] rounded-3xl shadow-2xl max-w-sm text-center" initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}>
                        <h3 className="text-2xl font-bold text-[#E5C082] mb-4 uppercase">Support the Channel!</h3>
                        <p className="text-gray-400 mb-6 text-sm">To Like, Comment, or Subscribe, please visit the video on YouTube.</p>
                        <a href={videoUrl} target="_blank" rel="noopener noreferrer" className="block w-full py-3 bg-red-600 text-white font-bold rounded-xl mb-4 hover:bg-red-700 transition-colors">WATCH ON YOUTUBE</a>
                        <button onClick={() => setShowLikeAlert(false)} className="text-[#E5C082] text-sm font-bold border-b border-[#E5C082]">Close</button>
                    </motion.div>
                )}
            </AnimatePresence>
        </motion.div>
